            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
                {
                    ...
                    <div key={project.id} className="shadow-xl rounded-2xl cursor-pointer bg-bgShade">
                        <img src={project.image} alt="" className="hover:scale-90 transition-all duration-300 rounded-2xl" />
                        <div className="p-8">
                            <h3 className="text-2xl font-semibold mb-2 text-headingColor">{project.name}</h3>
                            <p className="text-body mb-4">{project.description}</p>
                            <a href={project.link} className="font-bold underline underline-offset-8 hover:text-primary "> View <img src="/src/assets/Vector.png" alt="" className="w-3 inline-block ml-3" /> </a>
                        </div>
                    </div>)
                }
            </div>
